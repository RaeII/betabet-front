...
export function useZoomPan(opts: ZoomPanOptions = {}) {
  ...
  /** Suprime o click que segue um drag, para o card não navegar sem querer. */
  ...
  // Posição inicial legível no mount; em resize só re-clampeia (preserva a
  // exploração do usuário). Se o conteúdo ainda não tem dimensões no 1º run,
  // o ResizeObserver inicializa quando elas chegam.
  useLayoutEffect(() => {
    const vp = viewportRef.current
    const ct = contentRef.current
    if (!vp || !ct) return
    let initialized = center()
    const ro = new ResizeObserver(() => {
      if (pointers.current.size > 0) return
      if (!initialized) initialized = center()
      else reclamp()
    })
    ro.observe(vp)
    ro.observe(ct)
